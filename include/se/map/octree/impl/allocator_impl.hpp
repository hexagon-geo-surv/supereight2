...
#endif
    for (int i = 0; i < static_cast<int>(voxel_coords.size()); i++) {
...
#pragma omp parallel for
        for (int i = 0; i < static_cast<int>(unique_voxel_keys_at_scale.size()); i++) {
...
#pragma omp parallel for
    for (int i = 0; i < static_cast<int>(unique_voxel_keys.size()); i++) {
