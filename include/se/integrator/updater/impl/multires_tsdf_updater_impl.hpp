...
template<se::Colour    ColB,
         ...
         typename SensorT
>
void Updater<Map<Data<se::Field::TSDF, ColB, SemB>, se::Res::Multi, BlockSize>, SensorT>::operator()(
...
#pragma omp parallel for
  for (unsigned int i = 0; i < block_ptrs.size(); i++) {
    ...
    const Eigen::Vector3f block_centre_point_S = (T_SW * block_centre_point_W.homogeneous()).head(3);
    ...
    const int curr_scale = std::max(
            sensor_.computeIntegrationScale(block_centre_point_S, map_.getRes(), last_curr_scale,
                                            block_ptr->getMinScale(), block_ptr->getMaxScale()),
    ...
    if (curr_scale < last_curr_scale) {
      ...
      auto child_down_funct = [&](const OctreeType &octree,
          ...
          if (child_data_union.data.weight != 0) {
            ...
            const Eigen::Vector3f child_sample_coord_f = se::get_sample_coord(child_data_union.coord,
                                                                              1 << child_data_union.scale);
            int child_scale_returned;
            auto interp_field_value = se::visitor::getFieldInterp(octree, child_sample_coord_f, child_data_union.scale,
                                                                  child_scale_returned);
      ...
      se::propagator::propagateBlockDown(*map_.getOctree(), static_cast<se::OctantBase *>(block_ptr), curr_scale,
                                         child_down_funct, parent_down_funct);
    }
    ...
    se::propagator::propagateBlockUp(*map_.getOctree(), static_cast<se::OctantBase *>(block_ptr), curr_scale,
                                     child_up_funct, parent_up_funct);
  }
...
template<se::Colour    ColB,
        ...
        typename SensorT
>
void Updater<Map<Data<se::Field::TSDF, ColB, SemB>, se::Res::Multi, BlockSize>, SensorT>::updateVoxel(
        typename BlockType::DataUnion &data_union,
