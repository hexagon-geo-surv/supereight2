...
{
    const int input_height = input_point_cloud_S.height();
    const int input_width = input_point_cloud_S.width();
    const int surface_width = surface_point_cloud_W_ref.width();
...
#pragma omp parallel for
    for (int y = 0; y < input_height; y++) {
        for (int x = 0; x < input_width; x++) {
            ...
            Data& row = output_data[pixel.x() + pixel.y() * surface_width];

            if (input_normals_S[pixel.x() + pixel.y() * input_width] == se::math::g_invalid_normal) {
                row.result = -1;
                continue;
            }

            // point_W := The input point in world frame
            const Eigen::Vector3f point_W = T_WS * input_point_cloud_S[pixel.x() + pixel.y() * input_width];
            ...
            if(!project_fn(point_S_ref, ref_pixel_f)){
                row.result = -2;
                continue;
            }
            ...
            const Eigen::Vector3f ref_normal_W =
                surface_normals_W_ref[ref_pixel.x() + ref_pixel.y() * surface_width];

            if (ref_normal_W == se::math::g_invalid_normal) {
                row.result = -3;
                continue;
            }

            const Eigen::Vector3f ref_point_W =
                surface_point_cloud_W_ref[ref_pixel.x() + ref_pixel.y() * surface_width];
            const Eigen::Vector3f diff = ref_point_W - point_W;
            const Eigen::Vector3f input_normal_W =
                T_WS.linear() * input_normals_S[pixel.x() + pixel.y() * input_width];
