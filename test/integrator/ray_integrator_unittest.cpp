...
static void expect_valid_node_data(const NodeType& node, const se::OctantBase* const child)
{
  ...
  if (min_data.observed) {
    EXPECT_LE(node.getMinData().occupancy * node.getMinData().weight , min_data.occupancy * min_data.weight) << failure_message.str();
  }
  if (max_data.observed) {
    EXPECT_GE(node.getMaxData().occupancy *  node.getMaxData().weight, max_data.occupancy * max_data.weight) << failure_message.str();
  }
}
    ...
    for (int y = 0; y < stride; y += stride / 2) {
      for (int x = 0; x < stride; x += stride / 2) {
        ...
        if (child_min_data.observed) {
          EXPECT_LE(parent_min_data.occupancy * parent_min_data.weight, child_min_data.occupancy * child_min_data.weight)
                        << failure_message.str();
        }
        if (child_max_data.observed) {
          EXPECT_GE(parent_max_data.occupancy * parent_max_data.weight, child_max_data.occupancy * child_max_data.weight)
                        << failure_message.str();
        }
      }
    }
...
TEST(RayIntegrator, SingleRay){
  ...
  while(di <= d){
    ...
    const se::OctantBase* const finest_octant_ptr = se::fetcher::finest_octant<OctreeType>(voxel_coordinates, 0, map.getOctree().getRoot());
    EXPECT_TRUE(finest_octant_ptr->isBlock());

    const OctreeType::BlockType* const block_ptr = static_cast<const OctreeType::BlockType* const>(finest_octant_ptr);
    ...
    int computedIntegrationScale = sensor.computeIntegrationScale(block_centre_point, res, -1, -1, 3);
