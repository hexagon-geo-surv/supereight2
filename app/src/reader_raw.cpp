...
se::ReaderStatus se::RAWReader::nextColour(se::Image<rgb_t>& colour_image)
{
    // Seek to the appropriate place in the file.
    ...
    if ((colour_image.width() != size.x()) || (colour_image.height() != size.y())) {
        colour_image = se::Image<rgb_t>(size.x(), size.y());
    }
    // Read the whole image into a buffer.
    ...
    std::memcpy(colour_image.data(), buffer.data(), image_size * sizeof(rgb_t));
    return se::ReaderStatus::ok;
}
