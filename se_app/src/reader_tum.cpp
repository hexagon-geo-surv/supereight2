...
struct TUMPoseEntry {
  ...
  TUMPoseEntry(const std::string& s) {
    ...
    orientation = Eigen::Quaternionf(
        std::stof(columns[7]), std::stof(columns[4]), std::stof(columns[5]), std::stof(columns[6]));
  }
 ...
 * match was found.
 */
std::pair<TUMPoseEntry, TUMPoseEntry> surrounding_poses(
    const double                     query_timestamp,
    const std::vector<TUMPoseEntry>& poses_to_search) {
...
void associate_images(std::vector<TUMImageEntry>& depth_images,
  ...
  std::transform(depth_images.begin(), depth_images.end(),
      depth_timestamps.begin(), [](const auto& e) { return e.timestamp; });
  ...
  std::transform(rgb_images.begin(), rgb_images.end(),
      rgb_timestamps.begin(), [](const auto& e) { return e.timestamp; });
  ...
  for (size_t i = 0; i < depth_timestamps.size(); ++i) {
    ...
    if (timestamp_dist <= max_timestamp_dist) {
      ...
      std::cerr << "Warning: Could not match depth image "
          << depth_images[i].filename
          << " to any RGB image.\nThe closest timestamp distance was "
 ...
 */
std::vector<TUMPoseEntry> interpolate_poses(
    std::vector<TUMImageEntry>&      depth_images,
    std::vector<TUMImageEntry>&      rgb_images,
  ...
  for (size_t i = 0; i < depth_images.size(); ++i) {
    ...
    associated_poses.emplace_back(depth_timestamp, p, o,
        depth_images[i].filename, rgb_images[i].filename);
  }
...
se::TUMReader::TUMReader(const se::ReaderConfig& c)
    : se::Reader(c) {
  ...
  if (!stdfs::is_directory(sequence_path_)
      || !stdfs::is_directory(sequence_path_ + "/depth")
...
#pragma omp parallel for
  for (size_t i = 0; i < w * h; ++i) {
    ...
    depth_image[i] = inverse_scale_ * depth_value_unscaled;
  }
  free(image_data);
